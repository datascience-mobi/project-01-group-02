...
plot(cor.hc, las = 2, cex.lab = 2, main = "Clustering of the expression values of all cell lines")

rm(drivergene, dataset, cor.hc, cor.mat, cor.dist)
```

> * We hoped that the driver mutations cluster together, but they don`t. 
> * Because most of the timer there is more than one driver mutation in every cell line.

## <span style="color:orange">**3.2 K-means**</span>

* Performing a k-means.
* Using the kink method to identify the best number of clusters. 
...
* Two clusters seem to be the best. 
* But we want to take a look at different clusters.
...
* Performing a pca investigate what drives the difference in the clustering.

```{r}
dataset <- processed_data$expression # determining which dataset will be used
colnames(dataset)<- cellinesMutations$Genes # sets the labels to the Drivermutaion every Cell lines
...
Aim : identify combinations of one Drivergene with one other gene which is strongly correlated 

Data: Knockdown Data

Procedure: 

1. design a vector which contains the names of the 10 driver Mutaions 
2. design a 2nd vector which contains the names of all the genes in our Data 
3. design an iteration with the lapply function which compares one element of each of these vectors with one another
4. we will select combination of genes with the lowest p-value 

**Thereby we want to find genes with low p-values which indicates a high correlation. These Genes will be the subject of the further analysis steps.**
...
> The Genes you see in the graph are the most important genes for the drivergene TTN. 
...
Firstly we need to create a Dataframe with all the data we want to use in the multiple linear regression. 
...
Regressionanalysis <-lapply(1:10, function(x){
  ...
  regressor = lm( Driverexpression ~  copynumber + Value + kd.ceres + kd.prob , data = training_set) # predicting profit based on ALL (=.) the input variables for one company 
...
> * *Using a PCA and a p-test. *
> * *But the 2nd targets from the pca and the regression are not the same once.*
...
>In which context are our driver mutations to be put in and how do they interact with their second targets ?
...
- Function: connects the Z line to the M line in the sacromere 
	    â€”> contration of cardiac and skeletal muscle
...
NFIC: cellular transcription factors  
TADA2A: DNA-binding transcriptional activator proteins

# Network of TTN and second targets

![](C:/Users/LeoTh/Documents/GitHub/project-01-group-02/Bioinfobilder/genemania-network-2.jpg TTN.jpg)
...
- Location: RNA expression in all tissues 

# Protein expression of TP53

![](C:/Users/LeoTh/Documents/GitHub/project-01-group-02/Bioinfobilder/1.png)
...
![](C:/Users/LeoTh/Documents/GitHub/project-01-group-02/Bioinfobilder/skin cancer staining.png)

*arrows point at stained tissue with increased TP53 expression (10)*

> possible second targets of TP53 could be intresting target point for skin cancer theraphy


- Predicted second targets: CDKN1A, RAD50, TAF4
 
#Network of TP53 and second targets

![](C:/Users/LeoTh/Documents/GitHub/project-01-group-02/Bioinfobilder/TP53 .jpg)

*Network is generated by GeneMANIA is shown above*

>pathway of TP53 and CDKN1A are connected.  


#Network of pathways between TP53 and CDKN1A

![](C:/Users/LeoTh/Documents/GitHub/project-01-group-02/Bioinfobilder/genemania-network-3.jpg)

*pathways conecting TP53 and CDKN1A generated by GeneMANIA are shown above*

expression of CDKN1A is controlled by TP53,
â€”> mediates the p53-dependent cell cycle G1 phase arrest
...
- CDKN1A could be a possible sencond target in skin cancer therapy 
...
(10) Viros A, Sanchez-Laorden B, Pedersen M, et al. Ultraviolet radiation accelerates BRAF-driven melanomagenesis by targeting TP53. Nature. 2014;511(7510):478â€“482. doi:10.1038/nature13298
