...
```
#overview over our processed data -> outliers in every cellline, there are differences between the celllines which can be examined
...
```{r}
pheatmap(as.matrix(processed_data$kd.ceres[1:50, 1:34]), clustering_method = "ward.D2",border_color = "white", fontsize = 10, 
...
```
#wir hatten gehofft, dass sich die driver mutations zusammenclustern würden aber dies passiert nicht
...
ggplot(data = plotData) +
  ...
  theme(legend.position= "none", 
        ...
        strip.text.y = element_text(angle = 90)) 
```
#indentification of the outliers -> driver genes
